# 🤖 ANTIGRAVITY.md - Project Context & Guidelines

## 1. Project Identity

- **Name**: Infra Cheatsheets (Docs as Code)
- **Stack**: Python 3.x, MkDocs, Material for MkDocs Theme.
- **Goal**: DevOps 및 인프라 엔지니어를 위한 전반적인 인프라 기술(Kubernetes, Cloud, Linux, Network, Storage 등) 설치, 운영 및 트러블슈팅 가이드 제공.

## 2. Core Modes (Action Roles)

Antigravity, you must adopt one of the following modes based on the user's request:
...
- **Commit Messages**: 커밋 메시지는 가능한 한 **한글**로 작성하여 직관적으로 내용을 파악할 수 있게 하십시오. 또한, 모든 커밋 메시지 하단에는 반드시 `Co-Authored-By: Antigravity <noreply@google.com>`를 포함하여 협업 내역을 기록하십시오.
