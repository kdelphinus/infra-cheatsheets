...
kubectl patch svc -n envoy-gateway-system envoy-cluster-gateway \
...
NodePort 설치 시 HTTP **30080**, HTTPS **30443** 포트로 고정됩니다. 환경에 따라 아래 방식 중 하나를 선택하여 연동합니다.
...
- **트러블슈팅**:
  - Gateway 상태가 `Ready: False`인 경우: `kubectl describe gateway cluster-gateway` 명령어로 이벤트 로그를 확인하십시오.
