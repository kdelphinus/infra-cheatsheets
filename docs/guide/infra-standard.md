...
### 2.3. 에셋 및 다운로드 관리 표준 (Asset Management Standard)

폐쇄망 환경 구축에 필요한 모든 설치 파일(컨테이너 이미지, 바이너리, RPM/DEB 패키지 등)은 다음과 같은 원칙을 준수해야 해요.

1. **외부 공유 드라이브 업로드 금지**:
    - 컨테이너 이미지(`.tar`)나 패키지 파일을 구글 드라이브나 외부 개인 저장소에 직접 업로드하여 공유하는 방식은 엄격히 금지해요.
    - 이는 파일 유실, 버전 불일치, 외부 파일 도입으로 인한 보안 취약점 전파를 방지하기 위함이에요.
2. **에셋 수집 스크립트 제공**:
    - 모든 오프라인 패키지는 인터넷 연결이 가능한 외부망 호스트에서 직접 수집할 수 있도록 에셋 다운로드 스크립트(예: `scripts/download_assets_offline.sh` 또는 `ready-offline.md` 내 가이드)를 반드시 제공해야 해요.
    - 다운로드 스크립트는 공식 레포지토리나 신뢰할 수 있는 소스(GitHub 공식 릴리스 등)로부터 파일을 직접 다운로드하도록 설계해야 해요.
3. **패키징 재현성 확보**:
    - 스크립트 실행 후 수집된 파일들의 구조는 [1. Directory Structure](#1-directory-structure-디렉토리-표준-구조) 규격을 충족해야 하며, 언제 실행하더라도 동일한 구성 요소가 동일한 버전으로 수집되어 오프라인 환경에 재현될 수 있어야 해요.
